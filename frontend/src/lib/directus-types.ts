...
export interface Event {
	...
	description?: string | null;
	image?: DirectusFile | string | null;
	pretix_link?: string | null;
	status?: 'published' | 'draft' | 'archived' | null;
	is_highlight?: 'highlight' | null;
	recurrence?: 'none' | 'weekly' | 'biweekly' | 'monthly' | null;
	recurrence_end?: string | null;
	monthly_type?: 'date' | 'weekday' | null;
	categories?: EventsEventCategory[] | string[];
}

export interface EventsEventCategory {
	/** @primaryKey */
	id: number;
	events_id?: Event | string | null;
	event_categories_id?: EventCategory | string | null;
}
...
export interface Page {
	...
	hero_image?: DirectusFile | string | null;
	content?: string | null;
	status?: 'Live' | 'Draft' | 'Pending' | 'Scheduled' | 'Declined' | null;
}
...
export interface Service {
	...
	teaser?: string | null;
	description?: string | null;
	type?: `Freizeitassistenz (FASS)` | `Familienentlastungsdienst (FED)` | `Wohnassistenz (WASS)` | null;
	image?: DirectusFile | string | null;
	subtitle?: string | null;
	color?: string | null;
	program_file?: DirectusFile | string | null;
	button_text?: string | null;
	sort?: number;
	features?: unknown[] | null;
	status?: 'published' | 'draft' | 'rejected' | null;
	icon?: `fa-person-walking-luggage` | `fa-hands-holding-child` | `fa-house-chimney-user` | `fa-plane-departure` | `fa-hand-holding-heart` | `fa-circle-info` | `fa-star` | `fa-calendar-days` | null;
}
...
export interface Schema {
	...
	inbox_memberships: InboxMembership[];
	inbox_volunteers: InboxVolunteer[];
	pages: Page[];
	services: Service[];
...
export enum CollectionNames {
	...
	inbox_memberships = 'inbox_memberships',
	inbox_volunteers = 'inbox_volunteers',
	pages = 'pages',
	services = 'services',
